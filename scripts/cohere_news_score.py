...
def cohere_classify(api_key,news,prompt):
    ...
    embeddings_test = co.embed(texts=sentences_test,
                             model="large",
                             truncate="LEFT").embeddings
    embeddings_prompt = co.embed(texts=prompt,
                             model="large",
                             truncate="LEFT").embeddings
    svm_classifier = make_pipeline(StandardScaler(), SVC(class_weight='balanced')) 
    svm_classifier.fit(embeddings_train, labels_train)
    score = svm_classifier.score(embeddings_test, labels_test)
    prediction = svm_classifier.predict(embeddings_test)
    prompt_prediction = svm_classifier.predict(embeddings_prompt)

    return prediction, score, prompt_prediction[0]